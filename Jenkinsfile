       ...
       stage('Test PlayModeTests') {
        ...
        stage('Build IOS') {
            steps { 
                ...
                sh "${UNITY_PATH} -quit -batchmode -projectPath . -executeMethod BuildScript.PerformIOSBuild -stackTraceLogType Full --silent-crashes"
        ...
        stage('Build Android') {
            steps { 
                echo 'Bulding....'
                echo "Running ${BUILD_ID} on ${JENKINS_URL}"
                sh "${UNITY_PATH} -quit -batchmode -projectPath . -executeMethod BuildScript.PerformAndroidBuild -stackTraceLogType Full --silent-crashes"
