...
public abstract class GameGridBase : MonoBehaviour
{
    ...
    {
        int startX = (int)obj.GridPosition.x - Mathf.FloorToInt(width / 2); // obj.GetX() == CenterX ,   CenterX - width / 2 
        int startY = (int)obj.GridPosition.y - Mathf.FloorToInt(height / 2); // obj.GetY() == CenterY ,   CenterY - width / 2 
